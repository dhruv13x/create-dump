...
import create_dump
from create_dump.cleanup import safe_delete_paths, safe_cleanup
...
def test_safe_delete_paths_file_dry_run(mock_root):
    ...
    file_path.touch()
    with patch("create_dump.cleanup.logger") as mock_log:
...
def test_safe_delete_paths_file_delete(mock_root):
    file_path = mock_root / "test.md"
    file_path.touch()
    with patch("create_dump.cleanup.logger"):
...
def test_safe_delete_paths_dir_prompt_no(mock_root):
    ...
    dir_path.mkdir()
    with patch("create_dump.cleanup.confirm", return_value=False):
...
def test_safe_delete_paths_outside_root_skipped(mock_root):
    ...
    with patch("pathlib.Path.resolve", return_value=outside), \
         patch("create_dump.cleanup.safe_is_within", return_value=False), \
         patch("create_dump.cleanup.logger") as mock_log:
...
def test_safe_cleanup_no_matches(mock_root):
    with patch("create_dump.path_utils.find_matching_files", return_value=[]), \
         patch("create_dump.cleanup.logger") as mock_log:
...
def test_safe_cleanup_dry_run(mock_root):
    with patch("create_dump.path_utils.find_matching_files", return_value=[Path("fake.md")]), \
         patch("create_dump.cleanup.logger") as mock_log:
...
def test_safe_cleanup_confirm_no(mock_root):
    with patch("create_dump.path_utils.find_matching_files", return_value=[Path("fake.md")]), \
         patch("create_dump.cleanup.confirm", return_value=False), \
         patch("create_dump.cleanup.safe_delete_paths") as mock_delete, \
         patch("create_dump.cleanup.logger") as mock_log:
        safe_cleanup(mock_root, r".*test.*", dry_run=False, assume_yes=False, verbose=False)
...
def test_safe_cleanup_confirm_yes(mock_root):
    with patch("create_dump.path_utils.find_matching_files", return_value=[Path("fake.md")]), \
         patch("create_dump.cleanup.confirm", return_value=True), \
         patch("create_dump.cleanup.safe_delete_paths", return_value=(1, 0)) as mock_delete, \
         patch("create_dump.cleanup.logger") as mock_log:
        safe_cleanup(mock_root, r".*test.*", dry_run=False, assume_yes=False, verbose=False)
