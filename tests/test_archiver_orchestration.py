...
from code_dump.orchestrator import run_batch, _centralize_outputs
...
class TestCentralizeOutputs:
    ...
    def test_no_files(self, mock_root: Path, dump_pattern: str):
        with patch("code_dump.orchestrator.logger.info") as mock_log:
    ...
    def test_md_only(self, mock_root: Path, compress: bool):
        ...
        with patch("shutil.move") as mock_move, \
             patch("code_dump.orchestrator.logger.info") as mock_log:
    ...
    def test_test_unsafe_skip(self, mock_root: Path):
        ...
        with patch("code_dump.orchestrator.safe_is_within") as mock_within, \
             patch("code_dump.orchestrator.logger.warning") as mock_warn, \
...
class TestRunBatch:
    def test_no_sub_roots(self, mock_root: Path):
        ...
        with patch("code_dump.orchestrator.load_config") as mock_load:
            mock_load.return_value.dump_pattern = canonical_pattern
            with patch("code_dump.orchestrator.logger.warning") as mock_warn:
                run_batch(
                    root=mock_root,
                    subdirs=["nonexistent"],
    ...
    def test_pre_cleanup(self, mock_root: Path, pattern: str, expected_pattern: str):
        ...
        old_dump.touch()
        with patch("code_dump.orchestrator.find_matching_files") as mock_find, \
             patch("code_dump.orchestrator.confirm") as mock_confirm, \
             patch("code_dump.orchestrator.safe_delete_paths") as mock_delete, \
             patch("code_dump.orchestrator.load_config") as mock_load, \
             patch("code_dump.orchestrator.run_single") as mock_run_single:
    ...
    def test_success(self, mock_root: Path, pattern: str):
        ...
        with patch("code_dump.orchestrator.load_config") as mock_load, \
             patch("code_dump.orchestrator.run_single") as mock_run_single, \
             patch("code_dump.orchestrator._centralize_outputs") as mock_centralize, \
             patch("code_dump.orchestrator.DUMP_DURATION.time") as mock_time, \
             patch("code_dump.orchestrator.styled_print") as mock_print, \
             patch("code_dump.orchestrator.logger.info") as mock_info, \
             patch("code_dump.orchestrator.find_matching_files") as mock_find:
            mock_find.return_value = []
            mock_cfg = MagicMock(max_file_size_kb=100, use_gitignore=True, git_meta=True)
    ...
    def test_failure(self, mock_root: Path, pattern: str):
        ...
        with patch("code_dump.orchestrator.load_config") as mock_load, \
             patch("code_dump.orchestrator.run_single") as mock_run_single, \
             patch("code_dump.orchestrator.logger.error") as mock_error, \
             patch("code_dump.orchestrator.styled_print") as mock_print, \
             patch("code_dump.orchestrator.find_matching_files") as mock_find:
            mock_find.return_value = []
    ...
    def test_archive_integration_with_results(self, mock_root: Path, archive_results: dict):
        ...
        with patch("code_dump.orchestrator.load_config") as mock_load, \
             ...
             patch("code_dump.orchestrator.styled_print") as mock_print, \
             patch("code_dump.orchestrator.logger.info") as mock_info, \
             patch("code_dump.orchestrator.datetime") as mock_dt, \
             patch("code_dump.orchestrator.ArchiveManager") as mock_class, \
             patch("code_dump.orchestrator.find_matching_files") as mock_find:
            mock_find.return_value = []
            mock_cfg = MagicMock(max_file_size_kb=100, use_gitignore=True, git_meta=True, dump_pattern=DEFAULT_DUMP_PATTERN)
            mock_load.return_value = mock_cfg
